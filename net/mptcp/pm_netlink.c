...
{
	unsigned int addr_signaled, laddr_used, max;
...
static const struct nla_policy mptcp_pm_policy[MPTCP_PM_ATTR_MAX + 1] = {
	...
	[MPTCP_PM_ATTR_RCV_ADD_ADDR_MAX] = { .type	= NLA_U32,	},
};
...
static int mptcp_nl_add_addr(struct sk_buff *skb, struct genl_info *info)
{
...
static int mptcp_nl_del_addr(struct sk_buff *skb, struct genl_info *info)
{
	struct nlattr *attr = info->attrs[MPTCP_PM_ATTR_ADDR];
	struct pm_nl_pernet *pernet = genl_info_pm_nl(info);
	struct mptcp_pm_addr_entry addr, *entry;
...
static int mptcp_nl_flush_addrs(struct sk_buff *skb, struct genl_info *info)
{
	struct pm_nl_pernet *pernet = genl_info_pm_nl(info);
...
static int mptcp_nl_get_addr(struct sk_buff *skb, struct genl_info *info)
{
	struct nlattr *attr = info->attrs[MPTCP_PM_ATTR_ADDR];
	struct pm_nl_pernet *pernet = genl_info_pm_nl(info);
	struct mptcp_pm_addr_entry addr, *entry;
...
static int mptcp_nl_dump_addrs(struct sk_buff *msg, struct netlink_callback *cb)
{
...
static int
mptcp_nl_rcv_add_addr_max(struct sk_buff *skb, struct genl_info *info)
{
	struct nlattr *attr = info->attrs[MPTCP_PM_ATTR_RCV_ADD_ADDR_MAX];
	struct pm_nl_pernet *pernet = genl_info_pm_nl(info);
	int limit;
...
static struct genl_ops mptcp_pm_ops[] = {
	{
		...
		.doit   = mptcp_nl_add_addr,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = MPTCP_CMD_DEL_ADDR,
		.doit   = mptcp_nl_del_addr,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = MPTCP_CMD_FLUSH_ADDRS,
		.doit   = mptcp_nl_flush_addrs,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = MPTCP_CMD_GET_ADDR,
		.doit   = mptcp_nl_get_addr,
		.dumpit   = mptcp_nl_dump_addrs,
		.flags  = GENL_ADMIN_PERM,
	},
	{
		.cmd    = MPTCP_CMD_RCV_ADD_ADDR_MAX,
		.doit   = mptcp_nl_rcv_add_addr_max,
		.flags  = GENL_ADMIN_PERM,
	},
};
