...
chk_join_nr()
{
	...
	printf "%-50s %s" "$msg - syn"
	count=`NSTAT_HISTORY=$nstat ip netns exec $ns1 nstat |grep MPTcpExtMPJoinSynRx | awk '{print $2}'`
	[ -z "$count" ] && count=0
	if [ "$count" != "$syn_nr" ]; then
		echo "[fail] got $count JOIN[s] expected $syn_nr"
		ret=1
	else
		echo "[ ok ]"
	fi
	...
	printf "%50s %s" "- synack"
	count=`NSTAT_HISTORY=$nstat ip netns exec $ns2 nstat |grep MPTcpExtMPJoinSynAckRx | awk '{print $2}'`
	[ -z "$count" ] && count=0
	if [ "$count" != "$syn_ack_nr" ]; then
		echo "[fail] got $count JOIN[s] expected $syn_ack_nr"
		ret=1
	else
		echo "[ ok ]"
	fi
	...
	printf "%50s %s" "- ack"
	count=`NSTAT_HISTORY=$nstat ip netns exec $ns1 nstat |grep MPTcpExtMPJoinAckRx | awk '{print $2}'`
	[ -z "$count" ] && count=0
	if [ "$count" != "$ack_nr" ]; then
		echo "[fail] got $count JOIN[s] expected $ack_nr"
		ret=1
	else
		echo "[ ok ]"
	fi
