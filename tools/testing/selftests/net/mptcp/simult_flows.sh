...
setup()
{
	...
	capout=$(mktemp)
	size=$((2048 * 4096))
	dd if=/dev/zero of=$small bs=1024 count=1 >/dev/null 2>&1
...
do_transfer()
{
	...
	ip netns exec ${ns3} ./mptcp_connect -t $timeout -l -p $port 0.0.0.0 < "$sin" > "$sout" &
	local spid=$!
	...
	local start
	start=$(date +%s%3N)
	ip netns exec ${ns1} ./mptcp_connect -t $timeout -p $port 10.0.3.3 < "$cin" > "$cout" &
	local cpid=$!
